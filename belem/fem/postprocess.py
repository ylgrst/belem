...
def predict_plasticity_criterion_parameters(criterion: str, stress_tensor_dict: dict[str, npt.NDArray[np.float_]],
                                            plasticity_array_dict: dict[str, npt.NDArray[np.float_]],
    ...
    if criterion == "hill":
        ...
        raise ValueError("Plasticity criterion not implemented, choose either hill, dfa or anisotropic")

    def mse_criterion_params(p):
        mse = 0.0
        for load_case in stress_tensor_dict.keys():
            stress_tensor_at_plasticity_threshold = get_stress_tensor_at_plasticity_threshold(
